...
export function useSensors(hours = 24) {
  ...
  const fetch = useCallback(() => {
    setLoading(true);
    Promise.all([
      safeGet('/api/sensors/latest', null),
      safeGet(`/api/sensors/history?hours=${hours}`, []),
    ]).then(([nextLatest, nextHistory]) => {
      if (nextLatest) {
        setIsDemo(!!nextLatest.is_demo);
        setLatest(nextLatest);
      } else {
        setIsDemo(true);
        setLatest(latestFallback);
      }
      setHistory(nextHistory.length ? nextHistory : Array.from({ length: 12 }, (_, i) => ({
        recorded_at: `${i}:00`,
        soil_moisture: 31 + (i % 6), temperature: 27 + (i % 8),
        humidity: 56 + (i % 10), ph: 6.2 + (i % 4) / 10,
        nitrogen: 36 + i, phosphorus: 18 + (i % 6), potassium: 30 + (i % 9),
      })));
      setLoading(false);
    });
  }, [hours]);
  ...
  return { latest, history, loading, isDemo, refetch: fetch };
}
